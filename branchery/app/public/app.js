...
`).reverse().find(n=>n.startsWith(Dr));return t===void 0?"":t.slice(Dr.length).trim()}var Dr="\u2717";function Nr(e,t,n,r){let o=z(),i=new Map,d=new Set;async function u(h){if(!d.has(h)){d.add(h);try{let p=await t(h),b=p.status==="unknown"&&p.steps.length===0;i.set(h,{steps:Ie(yt(vt,p).steps),trouble:b?s("detail.noLog"):"",settled:!0,stopped:xo(p)})}catch(p){i.set(h,{steps:[],trouble:`${s("detail.logFailed")} ${k(p)}`,settled:!1,stopped:null})}finally{d.delete(h)}r()}}function m(h,p){let b=h.target;!(b instanceof Element)||!b.closest(".sds-run__head")||i.get(p)?.settled===!0||u(p)}function g(h){let p=i.get(h.id),b=`${C(h.started,l.language)} \xB7 ${Se(h.elapsed)}`;return a`
        <sds-run
            ...
            .stateWords=${Ye()}
            .steps=${p?.steps??[]}
            @click=${O=>m(O,h.id)}></sds-run>`}return{about(h){o.about(h)&&(i.clear(),q(o,h,()=>e(h),n))},forget(h){o.stillOn(h)&&(o.forget(h),i.clear())},draw(h){let p=o.trouble(h);if(p!=="")return a`<sds-note tone="warn" body=${`${s("detail.historyFailed")} ${p}`}></sds-note>`;let b=o.of(h);return b===null?L():b.length===0?a`<p class="branchery-list__quiet">${s("detail.noHistory")}</p>`:a`<div class="branchery-history">${b.map(g)}</div>`},lastFailed:h=>o.of(h)?.find(p=>p.status==="failed")??null,stoppedIn:h=>i.get(h)?.stopped??null,holds:h=>i.has(h),read:h=>void u(h)}}function xo(e){let t=e.steps.find(n=>n.state==="failed");return e.status!=="failed"||t===void 0?null:{no:t.no,step:t.label,reason:$t(e.log)}}function Xt(e){return{name:e,list:{read:null,trouble:"",open:!1},all:!1,diffs:new Map}}var en=class extends E{name="";handlers;usage=z();files=Xt("");reading=pe(k,()=>this.requestUpdate());log=pt((t,n)=>f.commits(t,n),this.reading,()=>this.requestUpdate());past=Nr(t=>f.worktreeJobs(t),t=>f.job(t),this.reading,()=>this.requestUpdate());bar=null;arrived(){this.untilLeft(Wr(()=>{this.files.list.open=!1,this.requestUpdate()})),this.untilLeft(Vn(t=>this.operationEnded(t)))}left(){jr()}operationEnded(t){if(this.past.forget(t),this.log.forget(t),this.files.name===t){let n=Er()===t;this.files=Xt(n?t:""),n&&(this.files.list.open=!0,this.readChanges(t))}this.usage.forget(t),this.name===t&&this.requestUpdate()}get worktree(){return[l.project,...l.worktrees].find(t=>t?.name===this.name)??null}willUpdate(){let t=this.name,n=this.worktree;this.past.about(t);let r=n?.incomplete===!0?this.past.lastFailed(t):null;r!==null&&!this.past.holds(r.id)&&this.past.read(r.id),n!==null&&this.log.about(t),n!==null&&!n.isProject&&this.usage.about(t)&&this.readUsage(t)}render(){let t=this.worktree;return t===null?ko(this.name):this.page(t)}updated(){this.files.name===this.name&&this.files.list.open&&Pr(this.name,s("table.uncommitted"),this.changeList(this.name))}page(t){let n=ne(t.name);return a`
      <div class="sds-bands">
        <section class="sds-band">
            ${l.error===""?c:a`<sds-note tone="error" body=${l.error}></sds-note>`}
            ${N(s("detail.back"),"#/")}
        ...
        <section class="sds-band sds-band--quiet">
            ...
            <div class="sds-facts-set">${Ar(t,{value:this.usage.of(t.name),trouble:this.usage.trouble(t.name)}).map(ht)}</div>
        </section>
      ...
      <div class="sds-bands">
        ...
        <section class="sds-band">
            ...
            ${n===""?this.log.body(t,r=>`#/b/${encodeURIComponent(t)}/c/${r}`):a`<sds-note tone="warn" body=${n}></sds-note>`}
                               ...
                               tone="neutral"></sds-badge>`:e.gone?a`<sds-badge label=${s("table.gone")} tone="warn"></sds-badge>`:!e.onRemote&&l.remotes.length>0?a`<sds-badge label=${s("table.nowhere")} tone="warn"></sds-badge>`:c}function Co(e){return[{title:s("detail.repository"),facts:[...e.base===null?[]:[{label:s("detail.base"),value:e.base.branch},{label:s("detail.sinceBase"),value:mt(e.base),said:!0}],{label:s("detail.commits"),value:Ao(e),said:!0},{label:s("detail.moved"),value:C(e.when,l.language),said:!0}]}]}function Ao(e){if(e.gone)return s("table.gone");if(e.upstream===null)return e.onRemote?s("detail.onRemoteOnly"):s("detail.noRemote");let t=[...e.ahead!==null&&e.ahead>0?[s("table.unpushed",{count:e.ahead})]:[],...e.behind!==null&&e.behind>0?[s("table.behind",{count:e.behind})]:[]];return t.length===0?s("detail.inStep"):`${e.upstream} \xB7 ${t.join(" \xB7 ")}`}var Eo={schedule:(e,t)=>setTimeout(e,t),cancel:e=>clearTimeout(e)};function Jr(e,t,n,r=Eo){let o=!1,i=null,d=()=>{i=r.schedule(()=>{i=null,u()},n)},u=async()=>{let m;try{m=await e()}catch{o||d();return}o||(t(m)?d():o=!0)};return u(),()=>{o=!0,i!==null&&(r.cancel(i),i=null)}}var Po=1e3,nn=0,Mr=null,sn=null;function on(e,t,n,r,o=!0){let i=++nn;Mr?.();let d=Hn(e,t,n??"create"),u=o,m=vt,g=()=>{u&&!nt()&&Ir(d)};sn=()=>{u=!0,tt(),Ir(d)},o&&sn();let h=async()=>{await A(),i===nn&&(l.job=d,g(),r(d))};Mr=Jr(()=>f.job(e,m.size),p=>i!==nn?!1:(m=yt(m,p),d={...p,log:m.log,steps:m.steps,expected:t??(p.subject===""?null:p.subject),kind:n??Dn(p.command)},p.status==="running"?(g(),!0):(h(),!1)),Po)}var jo={running:"job.running",done:"job.ticket.done",failed:"job.ticket.failed",unknown:"job.ticket.unknown"},G=null,rn="";function wt(){let e=l.job!==null&&!F()?l.job.status:"",t=jo[e];if(t===void 0){G?.remove(),G=null,rn="";return}G!==null&&rn===e||(G?.remove(),rn=e,G=x(s(t),"secondary",()=>{(sn??tt)(),wt()}),G.className=`branchery-ticket branchery-ticket--${e}`,G.title=s("job.show"),document.body.append(G))}ue(()=>wt());le(()=>wt());function Ir(e){ot(Oo(e),e.status==="running"?"":Bo(e)),er(a`
        <sds-run open
                 heading=${Lo(e)}
                 verdict=${e.status}
                 note=${Uo(e)}
...
`).filter(i=>i.includes(" -> ")).length;return o===0?s("job.done.fetch",{time:t}):s("job.done.fetchMoved",{count:o,time:t})}let n=l.worktrees.find(o=>o.name===e.expected);if(!n||e.kind==="remove")return s(`job.done.${e.kind}`,{name:e.expected??"",time:t});let r=e.kind==="sync"?s("job.done.sync",{name:n.database}):e.kind==="pull"?s("job.done.pull",{branch:n.branch}):e.kind==="restore"?s("job.done.restore",{branch:n.branch}):e.kind==="discard"?s("job.done.discard",{branch:n.branch}):n.entrypoints.length===0?s("job.done.built",{php:n.php}):`${s("job.done.built",{php:n.php})} ${s("job.login",Re)}`;return a`
        ...
        <sds-link external href=${n.url}
                  label=${s("action.openWorktree")}></sds-link>`}async function Ho(e){let t=S("#wizBack");try{await navigator.clipboard.writeText(e.log.trim()),t&&(ie(t,s("action.copied")),window.setTimeout(()=>ie(t,s("action.copyLog")),2e3))}catch{}}var St={onJob(e,t=null,n=null){on(e,t,n,Vr)}};function Vr(e){let t=e.expected??e.subject;t!==""&&Gn(t)}function Ee(e,t,n,r){let o=$(e);if(o.hidden=!t,!t)return;let i=o.firstElementChild;i===null&&(i=document.createElement("sds-note"),r!==void 0&&i.addEventListener("sds-note-action",r),o.append(i));for(let[d,u]of Object.entries(n()))i.setAttribute(d,u)}function Do(){Ee("#offline",l.unreachable,()=>({tone:"warn",body:s("error.unreachable"),action:s("action.tryAgain")}),()=>void A())}function Fo(){Ee("#update",l.updateWaiting,()=>({tone:"info",heading:s("update.waiting"),body:s("update.how")}))}function No(){let e=l.exposed;Ee("#exposed",e!==null,()=>({tone:"warn",heading:s("exposed.heading"),body:s(e==="router"?"exposed.router":"exposed.container")}))}function Jo(){Ee("#unconfigured",l.unconfigured,()=>({tone:"info",heading:s("error.unconfigured"),body:s("error.unconfiguredHow")}))}function Mo(){let e=l.recipeProblem;Ee("#recipe",e!==null,()=>({tone:"warn",heading:s("error.recipe"),body:e??""}))}var zr=Ze();function ge(e=Ze()){if(F())return;let t=!Mn(e,zr);t&&window.scrollTo(0,0),zr=e,Yr(),Do(),No(),Fo(),Qo(),Mo(),Jo(),Io(e),t&&qo()}function Io(e){let t=$("#main");y(zo(e),t);for(let n of t.children)n instanceof E&&n.drawNow()}function zo(e){return e.view==="worktree"?a`<branchery-worktree .name=${e.name} .handlers=${St}></branchery-worktree>`:e.view==="branch"?a`<branchery-branch .name=${e.name} .handlers=${St}></branchery-branch>`:e.view==="commit"?a`<branchery-commit
            .name=${e.name}
            .sha=${e.sha}
            .branch=${e.branch}></branchery-commit>`:a`<branchery-overview .handlers=${St}></branchery-overview>`}function qo(){$("#main").focus({preventScroll:!0})}le(()=>ge());qn(e=>{U(""),ge(e)});var fe=$("#bar"),Ae=[],Ko="https://benjaminkott.github.io/ddev-branchery/",qr="";function Yr(){qr!==l.language&&(qr=l.language,fe.menu={label:s("app.title"),items:[{label:s("nav.worktrees"),href:"#/",current:!0},{label:s("nav.docs"),href:Ko,external:!0}]})}function Zr(){fe.product=s("app.title"),Yr()}function Qr(){fe.languages=Ae.map(e=>({label:Go(e),current:e===l.language,lang:e})),fe.updateComplete.then(()=>{S(".sds-bar__lang",fe)?.setAttribute("name",s("app.language"))})}function Go(e){try{return new Intl.DisplayNames([e],{type:"language"}).of(e)??e.toUpperCase()}catch{return e.toUpperCase()}}fe.addEventListener("sds-dropdown-choose",e=>{let t=Ae[e.detail.index];!t||t===l.language||Wt(t).then(()=>{Qr(),Zr(),ge()})});$("#wizard").addEventListener("close",()=>{l.job?.status!=="running"&&(l.job=null)});ue(()=>{let e=In(location.hash);e!==null&&history.replaceState(null,"",e),ge(),A(!0)});var Vo=5e3,Kr=Date.now();async function Xr(){let e=Date.now();document.visibilityState!=="visible"||l.job?.status==="running"||F()||e-Kr<Vo||(Kr=e,es(await A(!0)))}function es(e){let t=e[0];t!==void 0&&l.job?.status!=="running"&&on(t.id,null,null,Vr,!1)}var Yo=2e3,Zo=1e4,Gr=!1;function Qo(){if(Gr)return;Gr=!0;let e=()=>{let t=l.runningJobs.length>0||F()||l.unreachable;window.setTimeout(()=>{if(document.visibilityState!=="visible"||F()){e();return}A(!0).then(e)},t?Yo:Zo)};e()}document.addEventListener("visibilitychange",()=>void Xr());window.addEventListener("focus",()=>void Xr());function ts(){if(Ut(location.hash)){I(St);return}nt()&&B()}window.addEventListener("hashchange",ts);(async()=>(Ae=await jn(),await Wt(Ae.includes(l.language)?l.language:Ae[0]??"en"),Qr(),Zr(),ge(),es(await A()),ge(),ts()))();
